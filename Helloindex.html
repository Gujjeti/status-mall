<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Status India's Tallest Mall</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">


    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Jost:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css" />
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&icon_names=ramen_dining" />

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/magnific-popup.js/1.2.0/magnific-popup.min.css" integrity="sha512-lvaVbvmbHhG8cmfivxLRhemYlTT60Ly9Cc35USrpi8/m+Lf/f/T8x9kEIQq47cRj1VQIFuxTxxCcvqiQeQSHjQ==" crossorigin="anonymous" referrerpolicy="no-referrer" />

    <link href="css/styles.css" rel="stylesheet" >


    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/gsap.min.js" integrity="sha512-7eHRwcbYkK4d9g/6tD/mhkf++eoTHwpNM9woBxtPUBWm67zeAfFC+HrdoE2GanKeocly/VxeLvIqwvCdk7qScg==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>

    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/ScrollTrigger.min.js" integrity="sha512-onMTRKJBKz8M1TnqqDuGBlowlH0ohFzMXYRNebz+yOcc5TQr/zAKsthzhuv0hiyUKEiQEQXEynnXCvNTOk50dg==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>

    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.14.5/dist/sweetalert2.all.min.js"></script>


  </head>
  <body>


    <header>
        <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center">
                    <div  class="menu-wrapper d-flex gap-2 align-items-center "  id="menuBtn">
                        <div class="menu-icon">
                           
        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M160-360v-80h640v80H160Zm0-160v-80h640v80H160Z"/></svg>
                        </div>
                        <span>Menu</span>
                    </div>
        
                    <div>
                        <a href="index.html" class="logo">
                            <img src="images/logo.png" class="logo" />
                        </a>
                    </div>
        
                    <a href="#contactUS">Contact</a>
                </div>
        </div>


        <div class="menu-list">
            <a href="#aboutus" >About Us
            </a>
            <a href="#spacesandfeatures" >Spaces and Features
            </a>
            <a href="#gallery" >Gallery</a>
            <a href="#locations" >Location Advantages</a>
            <a href="#contactUS">Contact Us</a>

            <button id="menuClose">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="m336-293.85 144-144 144 144L666.15-336l-144-144 144-144L624-666.15l-144 144-144-144L293.85-624l144 144-144 144L336-293.85ZM480.07-100q-78.84 0-148.21-29.92t-120.68-81.21q-51.31-51.29-81.25-120.63Q100-401.1 100-479.93q0-78.84 29.92-148.21t81.21-120.68q51.29-51.31 120.63-81.25Q401.1-860 479.93-860q78.84 0 148.21 29.92t120.68 81.21q51.31 51.29 81.25 120.63Q860-558.9 860-480.07q0 78.84-29.92 148.21t-81.21 120.68q-51.29 51.31-120.63 81.25Q558.9-100 480.07-100Zm-.07-60q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/></svg>
            </button>
        </div>
    </header>

    


    <section class="section-1">
        <div class="swiper mySwiper3">
            <div class="swiper-wrapper">
              <div class="swiper-slide overlay-black">

                <picture>
                    <source media="(min-width:650px)" srcset="images/banner2.png">
                    <img src="images/banner2-xs.jpg" class="img-fluid" >
                  </picture>


                <div class="slider-caption">
                    <h2 class="mb-3 heading-1 text-shadow">India's Tallest Mall</h2>
                    <p class="text-shadow">The world's most renowned designer brands will reside here. Couture, culture and commerce will blend effortlessly here.
                        Style will find unrivalled sophistication and scale here. A new shopping destination for Noida will rise here. This is Status Mall.
                        Now is your exclusive chance to invest and belong in the same circle.</p>
                </div>
              </div>

             

             
         
            </div>

            <div class="swiper-pagination"></div>
          </div>
    </section>



    <section class="p-5 section-2" id="aboutus">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <h2 class="heading-1 text_priamry">The space. <br/> The Ambition. <br/> The Investment.<br/> Opportunity. </h2>
                    <small class="d-block">We set the tallest standard.</small>

                    <ul class="list-group style-5 list-group-flush mt-5 p-0 m-0">
                        <li class="list-group-item text_grey pe-0 ps-0">A landmark project designed by acclaimed architects of world's biggest Dubai Mall.
                        </li>
                        <li class="list-group-item text_grey pe-0 ps-0">Primely located in a sought-after area frequented by Noida's elite.
                        </li>
                        <li class="list-group-item text_grey pe-0 ps-0">Elevated brand presence in a space synonymous with high-end retail.
                        </li>
                        <li class="list-group-item text_grey pe-0 ps-0">Early access to Noida's thriving growth story.
                        </li>
                        <li class="list-group-item text_grey pe-0 ps-0">Limited-time opportunity to secure prime spaces at exclusive rates.</li>
                      </ul>

                </div> 

                <div class="col-md-6">
                    <img src="images/girl-with-shopping-bags.jpg" class="img-fluid" />
                </div>
            </div>
        </div>
    </section>


    <section class="p-5 section-3">
        <div class="container-fluid">
            <div class="row align-items-center flex-lg-row-reverse">

               

                <div class="col-md-6">
                    <h2 class="heading-1 text_priamry">RARELY DOES <br/> OPULENCE COME  <br/> WITH SUCH<br/> AN OPPORTUNITY.
                    </h2>

                    <ul class="mt-5 p-0 m-0 list-unstyled list-style-2">
                        <li class="d-flex flex-column mb-3 ">
                        <span class="font-weight-bold">Project Name</span>
                        <span class="text_grey">Mogra Status
                        </span>
                        </li>

                        <li class="d-flex flex-column mb-3 ">
                            <span class="font-weight-bold">Developer</span>
                            <span class="text_grey">Mogra Realty

                            </span>
                            </li>

                            <li class="d-flex flex-column mb-3 ">
                                <span class="font-weight-bold">Designed By
                                </span>
                                <span class="text_grey">World-celebrated DP architects of Singapore

    
                                </span>
                                </li>

                                <li class="d-flex flex-column mb-3 ">
                                    <span class="font-weight-bold">Area</span>
                                    <span class="text_grey">1.4 million sq. ft.

        
                                    </span>
                                    </li>

                                    <li class="d-flex flex-column mb-3 ">
                                        <span class="font-weight-bold">Price</span>
                                        <span class="text_grey">On request

            
                                        </span>
                                        </li>

                                        <li class="d-flex flex-column mb-3 ">
                                            <span class="font-weight-bold">Retail Configurations</span>
                                            <span class="text_grey">Customisable, double-height spaces

                
                                            </span>
                                            </li>

                                            <li class="d-flex flex-column mb-3 ">
                                                <span class="font-weight-bold">Current Status
                                                </span>
                                                <span class="text_grey">New Phase Underway
                    
                                                </span>
                                                </li>

                      </ul>

                </div> 

                <div class="col-md-6">
                    <img src="images/watchman.jpg" class="img-fluid" />
                </div>

               
            </div>
        </div>
    </section>

    <section class="p-5 retail-sec section-4">
        <div class="container-fluid">

            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading-1 text-center mb-5 text_priamry">THE FINE ART OF RETAIL THERAPY. <br/>
                        TAKEN TO NEW HEIGHTS.
                        </h2>
                </div>
            </div>
           <div class="row justify-content-center mt-4">

         

            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M480-120q-74.31 0-140-28.42-65.69-28.43-114.42-77.16-48.73-48.73-77.16-114.42Q120-405.69 120-480q0-74.54 28.42-140.12 28.43-65.57 77.16-114.3 48.73-48.73 114.42-77.16Q405.69-840 480-840q74.54 0 140.12 28.42 65.57 28.43 114.3 77.16 48.73 48.73 77.16 114.3Q840-554.54 840-480q0 74.31-28.42 140-28.43 65.69-77.16 114.42-48.73 48.73-114.3 77.16Q554.54-120 480-120Zm0-39.69q35.23-45.23 58.08-88.85 22.84-43.61 37.15-97.61H384.77q15.85 57.07 37.92 100.69 22.08 43.61 57.31 85.77Zm-50.92-6q-28-33-51.12-81.58-23.11-48.58-34.42-98.88H190.15q34.39 74.61 97.5 122.38 63.12 47.77 141.43 58.08Zm101.84 0q78.31-10.31 141.43-58.08 63.11-47.77 97.5-122.38H616.46q-15.15 51.07-38.27 99.65-23.11 48.58-47.27 80.81ZM173.85-386.15h161.38q-4.54-24.62-6.42-47.97-1.89-23.34-1.89-45.88 0-22.54 1.89-45.88 1.88-23.35 6.42-47.97H173.85q-6.54 20.77-10.2 45.27Q160-504.08 160-480t3.65 48.58q3.66 24.5 10.2 45.27Zm201.38 0h209.54q4.54-24.62 6.42-47.2 1.89-22.57 1.89-46.65t-1.89-46.65q-1.88-22.58-6.42-47.2H375.23q-4.54 24.62-6.42 47.2-1.89 22.57-1.89 46.65t1.89 46.65q1.88 22.58 6.42 47.2Zm249.54 0h161.38q6.54-20.77 10.2-45.27Q800-455.92 800-480t-3.65-48.58q-3.66-24.5-10.2-45.27H624.77q4.54 24.62 6.42 47.97 1.89 23.34 1.89 45.88 0 22.54-1.89 45.88-1.88 23.35-6.42 47.97Zm-8.31-227.7h153.39Q734.69-690 673.5-736.23q-61.19-46.23-142.58-58.85 28 36.85 50.35 84.27 22.35 47.43 35.19 96.96Zm-231.69 0h190.46q-15.85-56.3-39.08-101.84-23.23-45.54-56.15-84.62-32.92 39.08-56.15 84.62-23.23 45.54-39.08 101.84Zm-194.62 0h153.39q12.84-49.53 35.19-96.96 22.35-47.42 50.35-84.27-82.16 12.62-142.96 59.23-60.81 46.62-95.97 122Z"/></svg>
                    </span>
                    <p>An unparalleled collection of global brands and exclusive experiences
                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M395.38-160h167.7v-45.54l22.3-7.69q76.54-23.69 130.12-76.73Q769.08-343 789.08-400H170.92q20 57 72.7 110.04 52.69 53.04 129.46 76.73l22.3 7.69V-160Zm-40 40v-57.69q-95.46-29.69-158.88-103.08Q133.08-354.15 120-440h80v-330l640-70v36.15l-423.08 46.62v84.15H840v35.39H416.92V-440H840q-11.54 85.85-76.5 159.23-64.96 73.39-160.42 103.08V-120h-247.7Zm-29.23-553.08h55.39v-80.46l-55.39 5.77v74.69Zm-90.77 0h55.39v-70.77l-55.39 6.31v64.46ZM326.15-440h55.39v-197.69h-55.39V-440Zm-90.77 0h55.39v-197.69h-55.39V-440ZM480-400Z"/></svg>
                    </span>
                    <p>Multiple gourmet dining options to delight every palate

                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M406.15-100v-244.62H295.38L408-628.92q9.23-22.16 28.62-35.08Q456-676.92 480-676.92q24 0 43.38 12.92 19.39 12.92 28.62 35.08l112.62 284.3H553.85V-100h-147.7ZM480-727.69q-27.92 0-47.81-19.89-19.88-19.88-19.88-47.8 0-27.93 19.88-47.81 19.89-19.89 47.81-19.89t47.81 19.89q19.88 19.88 19.88 47.81 0 27.92-19.88 47.8-19.89 19.89-47.81 19.89Z"/></svg>
                    </span>
                    <p>An exclusive floor reserved for designer
                        bridal-wear
                        
                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
<svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 -960 960 960"  fill="#5f6368"><path d="M480-132q-72 0-135.5-27.5T234-234q-47-47-74.5-110.5T132-480q0-72 27.5-135.5T234-726q47-47 110.5-74.5T480-828q72 0 135.5 27.5T726-726q47 47 74.5 110.5T828-480q0 72-27.5 135.5T726-234q-47 47-110.5 74.5T480-132Zm0-27q38-48 62-93t39-102H379q17 61 40 106t61 89Zm-35-3q-31-33-57-85.5T350-354H186q36 82 105 133t154 59Zm70 0q85-8 154-59t105-133H610q-17 55-43 107.5T515-162ZM175-382h169q-5-26-7-50.5t-2-47.5q0-23 2-47.5t7-50.5H175q-7 21-11 47t-4 51q0 25 4 51t11 47Zm197 0h216q5-26 7-49.5t2-48.5q0-25-2-48.5t-7-49.5H372q-5 26-7 49.5t-2 48.5q0 25 2 48.5t7 49.5Zm244 0h169q7-21 11-47t4-51q0-25-4-51t-11-47H616q5 26 7 50.5t2 47.5q0 23-2 47.5t-7 50.5Zm-6-224h164q-37-84-103.5-133T515-799q31 38 56 89t39 104Zm-231 0h202q-17-60-41.5-107.5T480-801q-35 40-59.5 87.5T379-606Zm-193 0h164q14-53 39-104t56-89q-90 11-156 60.5T186-606Z"/></svg>
                    </span>
                    <p>An entire floor reserved for a Gold
                        Souk
                        
                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M493.85-121.54q-62.39-7.42-123.93-32.55-61.54-25.14-111.5-73.29-49.96-48.16-83.04-122.08-33.07-73.92-38.46-178.23L135.38-560l30.77 1.54q41.77 2.31 99.23 21.08 57.47 18.76 97.54 47.84 5.08-75.23 37.2-158.42 32.11-83.19 79.88-141.27 47.77 58.08 79.88 142.04Q592-563.23 597.08-488q37.77-26.77 93.69-47.46 55.92-20.69 101.54-23l32.31-1.54-1.54 33.85q-3.08 82.77-25.77 148.61-22.69 65.85-64.19 115.54-41.5 49.69-101.08 84.42-59.58 34.73-138.19 56.04Zm-.47-40.46q-11-166-98.5-251t-217.5-105q11 171 101.5 255t214.5 101ZM480-363.69q13.46-25.08 34.58-52.43Q535.69-443.46 558-462q-.46-61.62-20.96-128.62T480-718.92q-36.54 61.3-57.04 128.3-20.5 67-20.96 128.62 22.31 18.54 43.15 45.88Q466-388.77 480-363.69Zm53.38 188.31q47-15.85 90.08-41.93 43.08-26.07 77.19-66.73 34.12-40.65 55.93-98.11 21.8-57.47 26.04-135.85-97.08 14-172.7 68.27-75.61 54.27-109 133.11 12 32 19.73 64.62 7.73 32.62 12.73 76.62ZM480-363.69Zm53.38 188.31Zm-40 13.38Zm7.54-154.62Zm-7.07 195.08Z"/></svg>
                    </span>
                    <p>Floor dedicated to salons and wellness
                        clinics
                        
                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M180-140v-680h40v80h120v-80h280v80h120v-80h40v680h-40v-80H620v80H340v-80H220v80h-40Zm40-120h120v-120H220v120Zm0-160h120v-120H220v120Zm0-160h120v-120H220v120Zm400 320h120v-120H620v120Zm0-160h120v-120H620v120Zm0-160h120v-120H620v120ZM380-180h200v-600H380v600Zm0-600h200-200Z"/></svg>
                    </span>
                    <p>A 5-screen multiplex ready to seat 1,000+ movie lovers

                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M209.23-240v60q0 8.5-5.75 14.25T189.23-160H180q-8.5 0-14.25-5.75T160-180v-293.85L239.38-700q2.93-9.54 11.27-14.77Q259-720 269.23-720h424.62q9.08 0 16.49 5.5 7.41 5.5 10.28 14.5L800-473.85V-180q0 8.5-5.75 14.25T780-160h-9.23q-8.5 0-14.25-5.75T750.77-180v-60H209.23Zm7.39-273.85h526.76L684.46-680H275.54l-58.92 166.15Zm-16.62 40V-280v-193.85Zm97.1 141.54q18.67 0 31.55-13.06 12.89-13.07 12.89-31.73 0-18.67-13.07-31.55-13.06-12.89-31.73-12.89-18.66 0-31.55 13.07-12.88 13.06-12.88 31.73 0 18.66 13.06 31.55 13.07 12.88 31.73 12.88Zm366.16 0q18.66 0 31.55-13.06 12.88-13.07 12.88-31.73 0-18.67-13.06-31.55-13.07-12.89-31.73-12.89-18.67 0-31.55 13.07-12.89 13.06-12.89 31.73 0 18.66 13.07 31.55 13.06 12.88 31.73 12.88ZM200-280h560v-193.85H200V-280Z"/></svg>
                    </span>
                    <p>Store-side parking across floors
                        4th to 9th
                        
                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M347.69-160v-40H460v-180.92q-86-13.23-135.31-75-49.31-61.77-49.31-144.08v-200h409.24v200q0 82.31-49.31 144.08-49.31 61.77-135.31 75V-200h112.31v40H347.69ZM480-420q62.92 0 109.54-46.31 46.61-46.31 53.69-113.69H316.77q7.08 67.38 53.69 113.69Q417.08-420 480-420ZM315.38-620h329.24v-140H315.38v140ZM480-420Z"/></svg>
                    </span>
                    <p>Rooftop restobars and dining overlooking the Expressway, Golf course & the Yamuna bank
                    </p>
                </div>
            </div>

            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M319.23-200h305.39v-400H540q-34.15 0-54.85 15.15-20.69 15.16-48.77 46-33.84 37.39-61.11 55.89-27.27 18.5-56.04 23.34V-200Zm-40 40v-300.62q-52-11.69-85.61-47.53Q160-544 160-600q0-46.85 27.81-83.62 27.81-36.76 75.81-52 21.46-46.46 61.57-75.3 40.12-28.85 94.81-28.85 30.38 0 59.35 10.46 28.96 10.46 57.03 35.08 8.47-3.54 16.31-5.04 7.85-1.5 17.31-1.5 56 0 93.38 42 37.39 42 37.39 98 0 18.92-2.42 33.54-2.43 14.61-8.04 27.23h37.38q29.93 0 51.12 20.81Q800-558.38 800-528.46v209.23q0 29.92-21.19 51.11-21.19 21.2-51.12 21.2h-63.07V-160H279.23ZM200-600q0 41.46 29.27 70.73Q258.54-500 300-500q35.85 0 60.27-20.23t48.42-46.23q28.08-30.85 56.89-52.19Q494.38-640 540.77-640h120q4.61-50.69-22.35-85.73-26.96-35.04-68.42-35.04-14.23 0-25.08 2.65-10.84 2.66-19.3 7.27l-22.54-19.07q-13.31-10.54-33.89-20.2-20.57-9.65-49.19-9.65-37.38 0-68.5 19.69-31.12 19.7-48.19 54.85l-10.93 22.31-24.3 7.92q-31.16 10.31-49.62 36.19Q200-632.92 200-600Zm464.62 313.08h63.07q13.85 0 23.08-9.23 9.23-9.23 9.23-23.08v-209.23q0-13.85-9.23-22.69-9.23-8.85-23.08-8.85h-63.07v273.08ZM319.23-200h305.39-305.39Z"/></svg>
                    </span>
                    <p>24/7 state-of-the-art microbreweries and premium lounges at higher floors

                    </p>
                </div>
            </div>


            <div class="col-md-3 col-6">
                <div class="box-icon pb-5">
                    <span class="icon">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M264.62-120q-27.62 0-46.12-18.5Q200-157 200-184.62v-430.76q0-27.62 18.5-46.12Q237-680 264.62-680H340v-20q0-58.31 40.85-99.15Q421.69-840 480-840t99.15 40.85Q620-758.31 620-700v20h75.38q27.62 0 46.12 18.5Q760-643 760-615.38v430.76q0 27.62-18.5 46.12Q723-120 695.38-120H264.62Zm0-40h430.76q9.24 0 16.93-7.69 7.69-7.69 7.69-16.93v-430.76q0-9.24-7.69-16.93-7.69-7.69-16.93-7.69H620v100q0 8.54-5.73 14.27T600-520q-8.54 0-14.27-5.73T580-540v-100H380v100q0 8.54-5.73 14.27T360-520q-8.54 0-14.27-5.73T340-540v-100h-75.38q-9.24 0-16.93 7.69-7.69 7.69-7.69 16.93v430.76q0 9.24 7.69 16.93 7.69 7.69 16.93 7.69ZM380-680h200v-20q0-42.23-28.88-71.12Q522.23-800 480-800q-42.23 0-71.12 28.88Q380-742.23 380-700v20ZM240-160v-480 480Z"/></svg>
                    </span>
                    <p>State-of-the-art security and smart shopping
                        solutions
                    </p>
                </div>
            </div>


           </div>
        </div>
    </section>





            <section class="px-0 ps-0 p-5 section-6 ">
                <div class="img-box position-relative ">

                    
                <picture>
                    <source media="(min-width:650px)" srcset="images/banner1.jpg">
                    <img src="images/banner1-xs.jpg" class="img-fluid h-full object-cover" >
                  </picture>

                    <h2 class="heading-1 text-white text-shadow">
                        Boutique spaces, <br/>
                        crafted for<br/>
                        high-end shopping
                    </h2>
                </div>
                    </section>



                    <section class="p-5 section-7" id="spacesandfeatures">
                        <div class="container-fluid">
                            <div class="row ">
                                <div class="col-md-6">


                                    <div class="accordion accordion-flush" id="accordionFlushExample">
                                        <div class="accordion-item">
                                          <h2 class="accordion-header">
                                            <button class="accordion-button " type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseGround" aria-expanded="true" aria-controls="flush-collapseGround">
                                                Ground floor
                                            </button>
                                          </h2>
                                          <div id="flush-collapseGround" class="accordion-collapse collapse show" data-bs-parent="#accordionFlushExample">
                                            <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6" >
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                        </span>
                                                        Super Market
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M269.23-200h-84.61q-26.66 0-45.64-18.98T120-264.62v-430.76q0-26.66 18.98-45.64T184.62-760H760v40H184.62q-10.77 0-17.7 6.92-6.92 6.93-6.92 17.7v430.76q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h84.61v40Zm166.15-75.38q25 0 42.5-17.5t17.5-42.5q0-25-17.5-42.5t-42.5-17.5q-25 0-42.5 17.5t-17.5 42.5q0 25 17.5 42.5t42.5 17.5ZM380-200v-57.92q-18.23-13.16-29.12-33.46-10.88-20.31-10.88-44 0-23.7 10.88-44 10.89-20.31 29.12-33.47v-57.92h110.77v57.92q18.23 13.16 29.11 33.47 10.89 20.3 10.89 44 0 23.69-10.89 44-10.88 20.3-29.11 33.46V-200H380Zm427.69 0H633.85q-13.73 0-23.02-9.29t-9.29-23.02v-335.38q0-13.73 9.29-23.02t23.02-9.29h173.84q13.73 0 23.02 9.29t9.29 23.02v335.38q0 13.73-9.29 23.02T807.69-200Zm-166.15-40H800v-320H641.54v320Zm0 0H800 641.54Z"/></svg>
                                                        </span>
                                                        Electronics 
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M321.77-244.62h110.77v-30.76h-80v-49.24h80v-110.76H321.77v30.76h80v49.24h-80v110.76Zm286.15 0h30.77v-190.76h-30.77v80h-49.23v-80h-30.77v110.76h80v80Zm212.31-279.53v319.53q0 27.62-18.5 46.12-18.5 18.5-46.11 18.5H204.85q-27.62 0-46.12-18.5-18.5-18.5-18.5-46.12v-321.07q-25.31-18.69-37.04-49t-.5-64.69l38.93-128.31q8-24.46 25.8-38.39Q185.23-820 211.46-820h536q26.23 0 43.92 13.04 17.7 13.04 25.93 37.73l40.46 129.85q11.23 34.38-.5 65.23-11.73 30.84-37.04 50Zm-252-15.85q38.54 0 57.54-21.58 19-21.57 16-44.57L615.15-780H500.23v168q0 29.46 20.15 50.73Q540.54-540 568.23-540Zm-180 0q32.23 0 52.12-21.27 19.88-21.27 19.88-50.73v-168H345.31l-26.62 176.92q-2.46 18.62 16.27 40.85T388.23-540Zm-178 0q26.46 0 44.96-18t23.04-44.92L303.31-780h-91.85q-13.08 0-20.77 5.77-7.69 5.77-11.54 17.31l-36.92 126.3q-9.85 31.54 8.42 61.08T210.23-540Zm540 0q35.92 0 57.39-28 21.46-28 10.61-62.62l-38.92-127.84Q775.46-770 767.77-775q-7.69-5-20.77-5h-89.85l25.08 177.08Q686.77-576 705.27-558t44.96 18ZM204.85-180h550.77q10.76 0 17.69-6.92 6.92-6.93 6.92-17.7v-300.46q-8.08 2.77-15.35 3.93-7.26 1.15-14.65 1.15-27 0-47.5-10.54t-38.96-33.61q-15.69 19.53-38.69 31.84-23 12.31-55.93 12.31-23.92 0-45.5-11.15-21.57-11.16-43.42-33-20.08 21.84-44.5 33Q411.31-500 389.15-500q-25.15 0-49.03-9.62-23.89-9.61-41.89-34.53-29.46 29.46-51.5 36.8-22.04 7.35-36.5 7.35-7.38 0-14.88-1.15-7.5-1.16-15.12-3.93v300.46q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92Zm550.77 0H204.85 755.62Z"/></svg>
                                                        </span>
                                                        Convenience stores
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M820.23-524.15v319.53q0 27.62-18.5 46.12-18.5 18.5-46.11 18.5H204.85q-27.62 0-46.12-18.5-18.5-18.5-18.5-46.12v-321.07q-25.31-18.69-37.04-49t-.5-64.69l38.93-128.31q8-24.46 25.8-38.39Q185.23-820 211.46-820h536q26.23 0 43.92 13.04 17.7 13.04 25.93 37.73l40.46 129.85q11.23 34.38-.5 65.23-11.73 30.84-37.04 50Zm-252-15.85q38.54 0 57.54-21.58 19-21.57 16-44.57L615.15-780H500.23v168q0 29.46 20.15 50.73Q540.54-540 568.23-540Zm-180 0q32.23 0 52.12-21.27 19.88-21.27 19.88-50.73v-168H345.31l-26.62 176.92q-2.46 18.62 16.27 40.85T388.23-540Zm-178 0q26.46 0 44.96-18t23.04-44.92L303.31-780h-91.85q-13.08 0-20.77 5.77-7.69 5.77-11.54 17.31l-36.92 126.3q-9.85 31.54 8.42 61.08T210.23-540Zm540 0q35.92 0 57.39-28 21.46-28 10.61-62.62l-38.92-127.84Q775.46-770 767.77-775q-7.69-5-20.77-5h-89.85l25.08 177.08Q686.77-576 705.27-558t44.96 18ZM204.85-180h550.77q10.76 0 17.69-6.92 6.92-6.93 6.92-17.7v-300.46q-8.08 2.77-15.35 3.93-7.26 1.15-14.65 1.15-27 0-47.5-10.54t-38.96-33.61q-15.69 19.53-38.69 31.84-23 12.31-55.93 12.31-23.92 0-45.5-11.15-21.57-11.16-43.42-33-20.08 21.84-44.5 33Q411.31-500 389.15-500q-25.15 0-49.03-9.62-23.89-9.61-41.89-34.53-29.46 29.46-51.5 36.8-22.04 7.35-36.5 7.35-7.38 0-14.88-1.15-7.5-1.16-15.12-3.93v300.46q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92Zm550.77 0H204.85 755.62Z"/></svg>
                                                        </span>
                                                       Department stores
                                                    </li>
                                                    </ul>
                                            </div>
                                          </div>
                                        </div>
                                        <div class="accordion-item">
                                          <h2 class="accordion-header">
                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseFirst" aria-expanded="false" aria-controls="flush-collapseFirst">
                                                1 <sup>st</sup>   floor
                                            </button>
                                          </h2>
                                          <div id="flush-collapseFirst" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                            <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                        </span>
                                                       Explore a paradise of men's apparels
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M269.23-200h-84.61q-26.66 0-45.64-18.98T120-264.62v-430.76q0-26.66 18.98-45.64T184.62-760H760v40H184.62q-10.77 0-17.7 6.92-6.92 6.93-6.92 17.7v430.76q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h84.61v40Zm166.15-75.38q25 0 42.5-17.5t17.5-42.5q0-25-17.5-42.5t-42.5-17.5q-25 0-42.5 17.5t-17.5 42.5q0 25 17.5 42.5t42.5 17.5ZM380-200v-57.92q-18.23-13.16-29.12-33.46-10.88-20.31-10.88-44 0-23.7 10.88-44 10.89-20.31 29.12-33.47v-57.92h110.77v57.92q18.23 13.16 29.11 33.47 10.89 20.3 10.89 44 0 23.69-10.89 44-10.88 20.3-29.11 33.46V-200H380Zm427.69 0H633.85q-13.73 0-23.02-9.29t-9.29-23.02v-335.38q0-13.73 9.29-23.02t23.02-9.29h173.84q13.73 0 23.02 9.29t9.29 23.02v335.38q0 13.73-9.29 23.02T807.69-200Zm-166.15-40H800v-320H641.54v320Zm0 0H800 641.54Z"/></svg>
                                                        </span>
                                                        footwear 
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M321.77-244.62h110.77v-30.76h-80v-49.24h80v-110.76H321.77v30.76h80v49.24h-80v110.76Zm286.15 0h30.77v-190.76h-30.77v80h-49.23v-80h-30.77v110.76h80v80Zm212.31-279.53v319.53q0 27.62-18.5 46.12-18.5 18.5-46.11 18.5H204.85q-27.62 0-46.12-18.5-18.5-18.5-18.5-46.12v-321.07q-25.31-18.69-37.04-49t-.5-64.69l38.93-128.31q8-24.46 25.8-38.39Q185.23-820 211.46-820h536q26.23 0 43.92 13.04 17.7 13.04 25.93 37.73l40.46 129.85q11.23 34.38-.5 65.23-11.73 30.84-37.04 50Zm-252-15.85q38.54 0 57.54-21.58 19-21.57 16-44.57L615.15-780H500.23v168q0 29.46 20.15 50.73Q540.54-540 568.23-540Zm-180 0q32.23 0 52.12-21.27 19.88-21.27 19.88-50.73v-168H345.31l-26.62 176.92q-2.46 18.62 16.27 40.85T388.23-540Zm-178 0q26.46 0 44.96-18t23.04-44.92L303.31-780h-91.85q-13.08 0-20.77 5.77-7.69 5.77-11.54 17.31l-36.92 126.3q-9.85 31.54 8.42 61.08T210.23-540Zm540 0q35.92 0 57.39-28 21.46-28 10.61-62.62l-38.92-127.84Q775.46-770 767.77-775q-7.69-5-20.77-5h-89.85l25.08 177.08Q686.77-576 705.27-558t44.96 18ZM204.85-180h550.77q10.76 0 17.69-6.92 6.92-6.93 6.92-17.7v-300.46q-8.08 2.77-15.35 3.93-7.26 1.15-14.65 1.15-27 0-47.5-10.54t-38.96-33.61q-15.69 19.53-38.69 31.84-23 12.31-55.93 12.31-23.92 0-45.5-11.15-21.57-11.16-43.42-33-20.08 21.84-44.5 33Q411.31-500 389.15-500q-25.15 0-49.03-9.62-23.89-9.61-41.89-34.53-29.46 29.46-51.5 36.8-22.04 7.35-36.5 7.35-7.38 0-14.88-1.15-7.5-1.16-15.12-3.93v300.46q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92Zm550.77 0H204.85 755.62Z"/></svg>
                                                        </span>
                                                        sports and denims
                                                    </li>
        
                                                   
                                                    </ul>
                                            </div>
                                          </div>
                                        </div>
                                        <div class="accordion-item">
                                          <h2 class="accordion-header">
                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseSecond" aria-expanded="false" aria-controls="flush-collapseSecond">
                                                2<sup>nd</sup> floor
                                            </button>
                                          </h2>
                                          <div id="flush-collapseSecond" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                            <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="m280-588.92-71.54 39.69q-11.69 6.46-23.08 3.61-11.38-2.84-17.84-14.53l-49.23-86.16q-6.46-11.69-3.62-23.46 2.85-11.77 14.54-18.23l190-112h48.46q11.31 0 17.96 6.65 6.66 6.66 6.66 17.97v7.69q0 35.31 26.19 61.5Q444.69-680 480-680t61.5-26.19q26.19-26.19 26.19-61.5v-7.69q0-11.31 6.66-17.97Q581-800 592.31-800h48.46l190 112q11.69 6.46 14.54 18.23 2.84 11.77-3.62 23.46l-49.23 86.16q-6.46 11.69-17.73 14.42-11.27 2.73-23.19-3.73L680-588.69v397.92q0 13.15-8.81 21.96T649.23-160H310.77q-13.15 0-21.96-8.81T280-190.77v-398.15ZM320-656v456h320v-456l122.46 67.23L802.92-658 631.69-759.54h-25.38q-3.46 50.23-39.19 84.89Q531.38-640 480-640t-87.12-34.65q-35.73-34.66-39.19-84.89h-25.38L157.08-658l40.46 69.23L320-656Zm160 176.23Z"/></svg>
                                                        </span>
                                                      Womens fashion apparel
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M260-840q-42.69 0-71.35 47.69Q160-744.62 160-680q0 57.62 18.65 113.81 18.66 56.19 45.97 93.27l86.15-21.23q14.54-32 31.88-84.93Q360-632 360-680q0-64.62-28.65-112.31Q302.69-840 260-840Zm55 520q19.77 0 32.38-14.77Q360-349.54 360-373q0-17-9.15-38.85-9.16-21.84-24.85-45.07l-86 21.54q0 52.3 19.42 83.84Q278.85-320 315-320Zm385-320q-42.69 0-71.35 47.69Q600-544.62 600-480q0 48 17.73 100.42 17.73 52.43 31.5 85.43l83.08 25.84q35.77-55.54 51.73-102.77Q800-418.31 800-480q0-64.62-28.65-112.31Q742.69-640 700-640Zm-55 520q34.62 0 54.81-30.77Q720-181.54 720-230l-85.23-27.69q-10.31 18.61-22.54 40.84Q600-194.62 600-173q0 20.77 13.27 36.88Q626.54-120 645-120ZM315-280q-53.92 0-85.08-42.38-31.15-42.39-29.92-107.24l-24.15-44.69q-20.23-31.61-38.04-88.54Q120-619.77 120-680q0-86.08 39.85-143.04Q199.69-880 260-880q61.15 0 100.58 59.35Q400-761.31 400-680q0 52.62-16.38 104.31-16.39 51.69-31.47 85.54l18 29.92Q382-440.85 391-416.88q9 23.96 9 43.88 0 40.08-23.96 66.54Q352.08-280 315-280ZM645-80q-37.08 0-61.04-26.46T560-173q0-19.92 9-43.88 9-23.97 20.85-43.35l18-29.92q-14.31-33.08-31.08-85.16Q560-427.38 560-480q0-81.31 39.42-140.65Q638.85-680 700-680q60.31 0 100.15 56.96Q840-566.08 840-480q0 60.23-17.42 116.27-17.43 56.04-38.43 89.19l-24.15 38q.23 53.31-25.42 104.92Q708.92-80 645-80Z"/></svg>
                                                        </span>
                                                       kids footwear 
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M406.15-100v-244.62H295.38L408-628.92q9.23-22.16 28.62-35.08Q456-676.92 480-676.92q24 0 43.38 12.92 19.39 12.92 28.62 35.08l112.62 284.3H553.85V-100h-147.7ZM480-727.69q-27.92 0-47.81-19.89-19.88-19.88-19.88-47.8 0-27.93 19.88-47.81 19.89-19.89 47.81-19.89t47.81 19.89q19.88 19.88 19.88 47.81 0 27.92-19.88 47.8-19.89 19.89-47.81 19.89Z"/></svg>
                                                        </span>
                                                       Cosmotics
                                                    </li>
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M240-200h147.69v-235.38h184.62V-200H720v-360L480-741.54 240-560v360Zm-40 40v-420l280-211.54L760-580v420H532.31v-235.38H427.69V-160H200Zm280-310.77Z"/></svg>
                                                        </span>
                                                       home decor
                                                    </li>
                                                   
                                                    </ul>
                                            </div>
                                          </div>
                                        </div>


                                        <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseThree" aria-expanded="false" aria-controls="flush-collapseThree">
                                                3<sup>rd</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapseThree" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M184.62-160q-26.85 0-45.74-18.88Q120-197.77 120-224.62v-110.76h720v110.76q0 26.85-18.88 45.74Q802.23-160 775.38-160H184.62ZM160-295.38v70.76q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h590.76q10.77 0 17.7-6.92 6.92-6.93 6.92-17.7v-70.76H160Zm320-160q-35.23 0-54.31 20-19.07 20-65.84 20t-65.62-20q-18.85-20-54.08-20t-54.3 20q-19.08 20-65.85 20v-40q35.23 0 54.31-20 19.07-20 65.84-20t65.62 20q18.85 20 54.08 20t54.3-20q19.08-20 65.85-20 46.77 0 65.85 20 19.07 20 54.3 20t53.31-20q18.08-20 64.85-20 46.77 0 67.07 20 20.31 20 54.62 20v40q-46.77 0-64.62-20-17.84-20-53.07-20-35.23 0-55.31 20-20.08 20-66.85 20-46.77 0-65.84-20-19.08-20-54.31-20Zm-360-120V-600q0-94.23 83.5-147.12Q287-800 480-800t276.5 52.88Q840-694.23 840-600v24.62H120ZM480-760q-156.31 0-234.81 38.31t-83.81 106.31h637.24q-5.31-68-83.81-106.31T480-760Zm0 464.62Zm0-320Z"/></svg>
                                                        </span>
                                                     Food
        
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M180.46-240q-30.23 0-47.08-22.04-16.84-22.04-12.92-49.19L164-615.85q6.69-45.38 39.65-74.77Q236.62-720 282-720h396q45.38 0 78.35 29.38 32.96 29.39 39.65 74.77l43.54 304.62q3.92 27.15-12.92 49.19Q809.77-240 779.54-240q-13.31 0-24-4.04t-20.31-13.65L632.92-360H327.08L224.77-257.69q-9.62 9.61-20.31 13.65-10.69 4.04-24 4.04Zm16-46 114-114h339.08l114 114q2 2 14.46 6 11 0 17.5-6.5T800-304l-44-308q-4-29-26.38-48.5Q707.23-680 678-680H282q-29.23 0-51.62 19.5Q208-641 204-612l-44 308q-2 11 4.5 17.5T182-280q3.54 0 14.46-6ZM680-447.69q13.15 0 22.73-9.58t9.58-22.73q0-13.15-9.58-22.73T680-512.31q-13.15 0-22.73 9.58T647.69-480q0 13.15 9.58 22.73t22.73 9.58Zm-80-120q13.15 0 22.73-9.58t9.58-22.73q0-13.15-9.58-22.73T600-632.31q-13.15 0-22.73 9.58T567.69-600q0 13.15 9.58 22.73t22.73 9.58ZM322.31-452.31h35.38v-70h70v-35.38h-70v-70h-35.38v70h-70v35.38h70v70ZM480-480Z"/></svg>
                                                        </span>
                                                        entertainment 
                                                    </li>
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>


                                          <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseFourth" aria-expanded="false" aria-controls="flush-collapseFourth">
                                                4<sup>th</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapseFourth" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M300-100v-361.38q-46.38-7.08-78.58-42.54-32.19-35.46-32.19-86.85V-860h40v269.23H300V-860h40v269.23h70.77V-860h40v269.23q0 51.39-32.19 86.85-32.2 35.46-78.58 42.54V-100h-40Zm369.23 0v-320h-98.46v-260q0-66.85 37.73-118.42 37.73-51.58 100.73-60.04V-100h-40Z"/></svg>
                                                        </span>
                                                        Refresh yourself with elite multiplex restaurants
        
                                                        </li>
        
                                                  
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>


                                          <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseFivth" aria-expanded="false" aria-controls="flush-collapseFivth">
                                                5<sup>th</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapseFivth" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-12">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M480-151.54 103.85-603.08 212.31-820h535.38l108.46 216.92L480-151.54ZM352.69-620h254.62l-80-160h-94.62l-80 160ZM460-237.62V-580H175.69L460-237.62Zm40 0L784.31-580H500v342.38ZM651.69-620h150.62l-80-160H571.69l80 160Zm-494 0h150.62l80-160H237.69l-80 160Z"/></svg>
                                                        </span>
                                                        From mesmerizing diamonds to vibrant colored gemstones, find your perfect piece of jewel right here.
        
        
                                                        </li>
                                                  
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>


                                          <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseSixth" aria-expanded="false" aria-controls="flush-collapseSixth">
                                                6<sup>th</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapseSixth" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-12">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M310-120v-226.92h-77.85q-30.06 0-51.11-21.28Q160-389.48 160-419.88q0-22.04 11.77-40.77 11.77-18.73 31.61-27.66L460-602.77v-49.85q-32.92-6.84-54.15-32.73-21.23-25.88-21.23-59.27 0-39.74 27.71-67.56Q440.05-840 479.64-840t67.28 28.08q27.7 28.07 27.7 67.3h-41.54q0-23.53-15.26-39.46Q502.56-800 480-800q-23.54 0-39.46 15.92-15.92 15.93-15.92 39.46 0 23.54 15.92 39.47 15.92 15.92 39.46 15.92 8.5 0 14.25 5.75t5.75 14.25v66.46l255.85 114.46q19.79 9.09 31.97 27.65Q800-442.11 800-420.14q0 30.29-21.27 51.76-21.27 21.46-51.65 21.46H650V-120H310Zm-77.85-266.92H310v-37.7h340v37.7h77.08q13.92 0 23.42-9.62 9.5-9.61 9.5-23.77 0-9.61-4.81-18.11t-14.42-12.81L480-567.38 220-451.23q-9.62 4.31-14.81 12.81-5.19 8.5-5.19 18.11 0 14.16 9.04 23.77 9.05 9.62 23.11 9.62ZM350-160h260v-224.62H350V-160Zm0-224.62h260-260Z"/></svg>
                                                        </span>
                                                        Choose your picture-perfect wedding outfit that would make everyone go WOW!
        
        
        
                                                        </li>
                                                  
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>


                                          <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseseventh" aria-expanded="false" aria-controls="flush-collapseseventh">
                                                7<sup>th</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapseseventh" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                        </span>
                                                        physical & mental health with Wellness hubs
        
        
                                                        </li>
        
                                                        <li class="col-md-6">
                                                            <span>
                                                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                            </span>
                                                            wellness clinics
            
            
                                                            </li>
        
                                                            <li class="col-md-6">
                                                                <span>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                                </span>
                                                                cosmetic centres
                
                
                                                                </li>
        
        
                                                                <li class="col-md-6">
                                                                    <span>
                                                                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M292.31-115.38q-25.31 0-42.66-17.35-17.34-17.35-17.34-42.65 0-25.31 17.34-42.66 17.35-17.34 42.66-17.34 25.31 0 42.65 17.34 17.35 17.35 17.35 42.66 0 25.3-17.35 42.65-17.34 17.35-42.65 17.35Zm375.38 0q-25.31 0-42.65-17.35-17.35-17.35-17.35-42.65 0-25.31 17.35-42.66 17.34-17.34 42.65-17.34t42.66 17.34q17.34 17.35 17.34 42.66 0 25.3-17.34 42.65-17.35 17.35-42.66 17.35ZM235.23-740 342-515.38h265.38q6.93 0 12.31-3.47 5.39-3.46 9.23-9.61l104.62-190q4.61-8.46.77-15-3.85-6.54-13.08-6.54h-486Zm-19.54-40h520.77q26.08 0 39.23 21.27 13.16 21.27 1.39 43.81l-114.31 208.3q-8.69 14.62-22.58 22.93-13.88 8.31-30.5 8.31H324l-48.62 89.23q-6.15 9.23-.38 20 5.77 10.77 17.31 10.77h435.38v40H292.31q-35 0-52.23-29.5-17.23-29.5-.85-59.27l60.15-107.23L152.31-820H80v-40h97.69l38 80ZM342-515.38h280-280Z"/></svg>
                                                                    </span>
                                                                    gym
                    
                    
                                                                    </li>
        
                                                  
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>


                                          <div class="accordion-item">
                                            <h2 class="accordion-header">
                                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapsenineth" aria-expanded="false" aria-controls="flush-collapsenineth">
                                                8<sup>th</sup> to  9<sup>th</sup> floor
                                              </button>
                                            </h2>
                                            <div id="flush-collapsenineth" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                                              <div class="accordion-body">
                                                <ul class="row list-unstyled floorAvailList ">
                                                    <li class="col-md-6">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M195.38-180v-40h80v-213.85L84.62-720h421.53L315.38-433.85V-220h80v40h-200Zm16-420h168l54.47-80H156.92l54.46 80Zm404.1 420q-41.63 0-70.86-29.14-29.24-29.14-29.24-70.77 0-41.63 29.17-70.86Q573.72-380 615.38-380q15.92 0 30.38 5.35 14.47 5.34 29.62 18.03V-720h160v70.77h-120V-280q0 41.67-29.14 70.83Q657.1-180 615.48-180Z"/></svg>
                                                        </span>
                                                        clubs pubs and bars
        
        
                                                        </li>
        
                                                   
                                                  
        
        
                                                   
                                                   
                                                    </ul>
                                              </div>
                                            </div>
                                          </div>

                                      </div>
                                  
                
                                 


                                    

                
                                </div> 
                
                                <div class="col-md-6  text-lg-end">
                                    <img src="images/headset.jpg" class="img-fluid" />
                                </div>
                            </div>
                        </div>
                    </section>




                    <section class="px-0 ps-0 p-5 section-8 pb-0">
                        <div class="img-box position-relative overlay-black">
                            

                            <picture>
                                <source media="(min-width:650px)" srcset="images/girl-in-black.jpg">
                                <img src="images/girl-in-black-xs.jpg" class="img-fluid h-full object-cover" >
                              </picture>

                            <h2 class="heading-1 text-white text-shadow">
                                Styled for the lavish life
                            </h2>
                        </div>
                            </section>



                            <section class="p-5 section-11"  id="gallery">
                                <div class="container-fluid">

                                    <div class="row align-items-center mb-5">
                                        <div class="col-md-12 text-center">
                                            <h2 class="heading-1 text_priamry">
                                                Gallery
                                                </h2>
                                           
                                            </div>
                                    </div>

                                    <div class="row align-items-center parent-container">
                                        <div class="col-md-3">
                                                <a href="images/gallery/IMG-20241113-WA0007.jpg">
                                                    <img src="images/gallery/IMG-20241113-WA0007.jpg" class="img-fluid"/>
                                                </a>
                                               
                                        </div>



                                
                                <div class="col-md-3">
                                    <a href="images/gallery/IMG-20241113-WA0010.jpg">
                                        <img src="images/gallery/IMG-20241113-WA0010.jpg" class="img-fluid"/>
                                    </a>
                                   
                            </div>


                            <div class="col-md-3">
                                <a href="images/gallery/IMG-20241113-WA0007.jpg">
                                    <img src="images/gallery/IMG-20241113-WA0007.jpg" class="img-fluid"/>
                                </a>
                               
                        </div>



                
                <div class="col-md-3">
                    <a href="images/gallery/IMG-20241113-WA0010.jpg">
                        <img src="images/gallery/IMG-20241113-WA0010.jpg" class="img-fluid"/>
                    </a>
                   
            </div>
                                        </div>
                                </div>
                                </section>


                            <section class="p-5 section-9 bg-img" style="background-image: url(images/highway.jpg);">
                                <div class="container-fluid">
                                    <div class="row align-items-center">
                                        <div class="col-md-12 text-center">
                                            <h2 class="heading-1 text-white mb-4">
                                                THE QUICKEST WAY TO THE TOP? <br/>
                                                TAKE THE EXPRESSWAY. 
                                            
                                            </h2>
                                       

                                            <p class="text-white mt-3 opacity-1">
                                                Primely located right on the 6-lane Noida Expressway - is a jewel. An investment<br/> opportunity that's highly coveted and expectedly rich with opportunity. Visible to everyone,<br/>
                                                but available only to an exclusive few. 
                                            </p>

                                           
                                            </div>
                                    </div>



                                    <div class="row justify-content-center mt-5">

         

                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M480-120q-74.31 0-140-28.42-65.69-28.43-114.42-77.16-48.73-48.73-77.16-114.42Q120-405.69 120-480q0-74.54 28.42-140.12 28.43-65.57 77.16-114.3 48.73-48.73 114.42-77.16Q405.69-840 480-840q74.54 0 140.12 28.42 65.57 28.43 114.3 77.16 48.73 48.73 77.16 114.3Q840-554.54 840-480q0 74.31-28.42 140-28.43 65.69-77.16 114.42-48.73 48.73-114.3 77.16Q554.54-120 480-120Zm0-39.69q35.23-45.23 58.08-88.85 22.84-43.61 37.15-97.61H384.77q15.85 57.07 37.92 100.69 22.08 43.61 57.31 85.77Zm-50.92-6q-28-33-51.12-81.58-23.11-48.58-34.42-98.88H190.15q34.39 74.61 97.5 122.38 63.12 47.77 141.43 58.08Zm101.84 0q78.31-10.31 141.43-58.08 63.11-47.77 97.5-122.38H616.46q-15.15 51.07-38.27 99.65-23.11 48.58-47.27 80.81ZM173.85-386.15h161.38q-4.54-24.62-6.42-47.97-1.89-23.34-1.89-45.88 0-22.54 1.89-45.88 1.88-23.35 6.42-47.97H173.85q-6.54 20.77-10.2 45.27Q160-504.08 160-480t3.65 48.58q3.66 24.5 10.2 45.27Zm201.38 0h209.54q4.54-24.62 6.42-47.2 1.89-22.57 1.89-46.65t-1.89-46.65q-1.88-22.58-6.42-47.2H375.23q-4.54 24.62-6.42 47.2-1.89 22.57-1.89 46.65t1.89 46.65q1.88 22.58 6.42 47.2Zm249.54 0h161.38q6.54-20.77 10.2-45.27Q800-455.92 800-480t-3.65-48.58q-3.66-24.5-10.2-45.27H624.77q4.54 24.62 6.42 47.97 1.89 23.34 1.89 45.88 0 22.54-1.89 45.88-1.88 23.35-6.42 47.97Zm-8.31-227.7h153.39Q734.69-690 673.5-736.23q-61.19-46.23-142.58-58.85 28 36.85 50.35 84.27 22.35 47.43 35.19 96.96Zm-231.69 0h190.46q-15.85-56.3-39.08-101.84-23.23-45.54-56.15-84.62-32.92 39.08-56.15 84.62-23.23 45.54-39.08 101.84Zm-194.62 0h153.39q12.84-49.53 35.19-96.96 22.35-47.42 50.35-84.27-82.16 12.62-142.96 59.23-60.81 46.62-95.97 122Z"/></svg>
                                                </span>
                                                <p>Over 1,00,000 vehicles ply
                                                    here daily
                                                </p>
                                            </div>
                                        </div>
                            
                            
                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M395.38-160h167.7v-45.54l22.3-7.69q76.54-23.69 130.12-76.73Q769.08-343 789.08-400H170.92q20 57 72.7 110.04 52.69 53.04 129.46 76.73l22.3 7.69V-160Zm-40 40v-57.69q-95.46-29.69-158.88-103.08Q133.08-354.15 120-440h80v-330l640-70v36.15l-423.08 46.62v84.15H840v35.39H416.92V-440H840q-11.54 85.85-76.5 159.23-64.96 73.39-160.42 103.08V-120h-247.7Zm-29.23-553.08h55.39v-80.46l-55.39 5.77v74.69Zm-90.77 0h55.39v-70.77l-55.39 6.31v64.46ZM326.15-440h55.39v-197.69h-55.39V-440Zm-90.77 0h55.39v-197.69h-55.39V-440ZM480-400Z"/></svg>
                                                </span>
                                                <p>A corporate hub comprising Software & Mobile App development sector
                                                    and Entertainment Sector
                                                    
                            
                                                </p>
                                            </div>
                                        </div>
                            
                            
                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M406.15-100v-244.62H295.38L408-628.92q9.23-22.16 28.62-35.08Q456-676.92 480-676.92q24 0 43.38 12.92 19.39 12.92 28.62 35.08l112.62 284.3H553.85V-100h-147.7ZM480-727.69q-27.92 0-47.81-19.89-19.88-19.88-19.88-47.8 0-27.93 19.88-47.81 19.89-19.89 47.81-19.89t47.81 19.89q19.88 19.88 19.88 47.81 0 27.92-19.88 47.8-19.89 19.89-47.81 19.89Z"/></svg>
                                                </span>
                                                <p>3 lakh plus upscale residents, with approx. 1.25 lakh new
                                                    units upcoming 
                                                    
                                                </p>
                                            </div>
                                        </div>
                            
                            
                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    
                            <svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 -960 960 960"  fill="#5f6368"><path d="M480-132q-72 0-135.5-27.5T234-234q-47-47-74.5-110.5T132-480q0-72 27.5-135.5T234-726q47-47 110.5-74.5T480-828q72 0 135.5 27.5T726-726q47 47 74.5 110.5T828-480q0 72-27.5 135.5T726-234q-47 47-110.5 74.5T480-132Zm0-27q38-48 62-93t39-102H379q17 61 40 106t61 89Zm-35-3q-31-33-57-85.5T350-354H186q36 82 105 133t154 59Zm70 0q85-8 154-59t105-133H610q-17 55-43 107.5T515-162ZM175-382h169q-5-26-7-50.5t-2-47.5q0-23 2-47.5t7-50.5H175q-7 21-11 47t-4 51q0 25 4 51t11 47Zm197 0h216q5-26 7-49.5t2-48.5q0-25-2-48.5t-7-49.5H372q-5 26-7 49.5t-2 48.5q0 25 2 48.5t7 49.5Zm244 0h169q7-21 11-47t4-51q0-25-4-51t-11-47H616q5 26 7 50.5t2 47.5q0 23-2 47.5t-7 50.5Zm-6-224h164q-37-84-103.5-133T515-799q31 38 56 89t39 104Zm-231 0h202q-17-60-41.5-107.5T480-801q-35 40-59.5 87.5T379-606Zm-193 0h164q14-53 39-104t56-89q-90 11-156 60.5T186-606Z"/></svg>
                                                </span>
                                                <p>Strategically located at the intersection of the Western and
                                                    Eastern Dedicated Freight Corridor
                                                </p>
                                            </div>
                                        </div>
                            
                            
                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M493.85-121.54q-62.39-7.42-123.93-32.55-61.54-25.14-111.5-73.29-49.96-48.16-83.04-122.08-33.07-73.92-38.46-178.23L135.38-560l30.77 1.54q41.77 2.31 99.23 21.08 57.47 18.76 97.54 47.84 5.08-75.23 37.2-158.42 32.11-83.19 79.88-141.27 47.77 58.08 79.88 142.04Q592-563.23 597.08-488q37.77-26.77 93.69-47.46 55.92-20.69 101.54-23l32.31-1.54-1.54 33.85q-3.08 82.77-25.77 148.61-22.69 65.85-64.19 115.54-41.5 49.69-101.08 84.42-59.58 34.73-138.19 56.04Zm-.47-40.46q-11-166-98.5-251t-217.5-105q11 171 101.5 255t214.5 101ZM480-363.69q13.46-25.08 34.58-52.43Q535.69-443.46 558-462q-.46-61.62-20.96-128.62T480-718.92q-36.54 61.3-57.04 128.3-20.5 67-20.96 128.62 22.31 18.54 43.15 45.88Q466-388.77 480-363.69Zm53.38 188.31q47-15.85 90.08-41.93 43.08-26.07 77.19-66.73 34.12-40.65 55.93-98.11 21.8-57.47 26.04-135.85-97.08 14-172.7 68.27-75.61 54.27-109 133.11 12 32 19.73 64.62 7.73 32.62 12.73 76.62ZM480-363.69Zm53.38 188.31Zm-40 13.38Zm7.54-154.62Zm-7.07 195.08Z"/></svg>
                                                </span>
                                                <p>World-class hospital, luxury hotels and over 100 Educational Institutions
                                                    in the neighbourhood 
                                                    
                                                </p>
                                            </div>
                                        </div>
                            
                            
                                        <div class="col-md-4">
                                            <div class="box-icon style-2">
                                                <span class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M180-140v-680h40v80h120v-80h280v80h120v-80h40v680h-40v-80H620v80H340v-80H220v80h-40Zm40-120h120v-120H220v120Zm0-160h120v-120H220v120Zm0-160h120v-120H220v120Zm400 320h120v-120H620v120Zm0-160h120v-120H620v120Zm0-160h120v-120H620v120ZM380-180h200v-600H380v600Zm0-600h200-200Z"/></svg>
                                                </span>
                                                <p>An estimated catchment of 2.1 Million population with
                                                    3000
                            
                                                </p>
                                            </div>
                                        </div>
                            
                            
                            
                            
                                       </div>


                                    </div>
                                    </section>



                            <section class="p-5 section-10" id="locations">
                                <div class="container-fluid">
                                    <div class="row align-items-center">
                                        <div class="col-md-12 text-center">
                                            <h2 class="heading-1 text_priamry border-bottom-style-1 mb-4">
                                                An Address so influential<br/>  commanding attention is easy</h2>
                                            <small class="d-block border-bottom-style-1 mb-4">Section 129, Noida Expressway</small>

                                            <p class="text_grey mt-3">
                                                Minutes away from premium residential communities.<br/>
                                                Undisturbed access to Jewar Airport, South Delhi via DND Flyway.
                                            </p>

                                            <img src="images/locations-map.png" class="img-fluid" />
                                            </div>
                                    </div>
                                    </div>
                                    </section>


                                        
    <section class="p-5 section-5 " id="contactUS">
        <div class="container-fluid">


            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading-1 text-center mb-5 text_priamry">Contact us
                        </h2>
                </div>
            </div>


            <div class="row  mt-3 mt-md-5">
                <div class="col-md-12">

                    
                
<div class="envelope">
    <div class="back">
        <div class="cover-text"></div>
    </div>
   
    <form class="letter" id="contactForm" method="post" action="contact.php" onsubmit=" validateForm(event)">
        <p>
            <label for="name">Name:</label>
            <input type="text" name="name" id="name" />
        </p>

        <p>
            <label for="mobile">Mobile:</label>
            <input type="text" name="mobile" id="mobile" pattern="[0-9]{10}" />
        </p>

        <p>
            <label for="email">Email:</label>
            <input type="email" name="email" id="email" />
        </p>

        <button type="submit" class="cta">
            <span>Submit</span>
            <svg width="13px" height="10px" viewBox="0 0 13 10">
                <path d="M1,5 L11,5"></path>
                <polyline points="8 1 12 5 8 9"></polyline>
            </svg>
        </button>
    </form>

    <div class="evn-logo">
        <img src="images/logo.png" />
    </div>

    <button class="close-env">
        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368">
            <path d="m336-307.69 144-144 144 144L652.31-336l-144-144 144-144L624-652.31l-144 144-144-144L307.69-624l144 144-144 144L336-307.69ZM480.13-120q-74.67 0-140.41-28.34-65.73-28.34-114.36-76.92-48.63-48.58-76.99-114.26Q120-405.19 120-479.87q0-74.67 28.34-140.41 28.34-65.73 76.92-114.36 48.58-48.63 114.26-76.99Q405.19-840 479.87-840q74.67 0 140.41 28.34 65.73 28.34 114.36 76.92 48.63 48.58 76.99 114.26Q840-554.81 840-480.13q0 74.67-28.34 140.41-28.34 65.73-76.92 114.36-48.58 48.63-114.26 76.99Q554.81-120 480.13-120Zm-.13-40q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/>
        </svg>
    </button>
</div>
                    
                    
                      
                </div>
            </div>
            </div>
            </section>
            
                    <footer>
                        <p class="p-0 mb-0"> &copy; 2024 Status India's Tallest Mall. All right reserved
                        </p>
                    </footer>              

            
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>

    <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert2/11.14.5/sweetalert2.min.js" integrity="sha512-JCDnPKShC1tVU4pNu5mhCEt6KWmHf0XPojB0OILRMkr89Eq9BHeBP+54oUlsmj8R5oWqmJstG1QoY6HkkKeUAg==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/magnific-popup.js/1.2.0/jquery.magnific-popup.min.js" integrity="sha512-fCRpXk4VumjVNtE0j+OyOqzPxF1eZwacU3kN3SsznRPWHgMTSSo7INc8aY03KQDBWztuMo5KjKzCFXI/a5rVYQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>

    <!-- Swiper JS -->
  <script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js"></script>







<script src="js/main.js"></script>


<script>
   
 // JavaScript validation function
function validateForm(event) {
    event.preventDefault();  // Prevent form submission (page reload)
   
    const name = document.getElementById('name').value.trim();
    const mobile = document.getElementById('mobile').value.trim();
    const email = document.getElementById('email').value.trim();

    if (!name) {
        Swal.fire('Validation Error', 'Please enter your name', 'error');
        return false;
    }

    if (!/^[0-9]{10}$/.test(mobile)) {
        Swal.fire('Validation Error', 'Please enter a valid 10-digit mobile number', 'error');
        return false;
    }

    if (!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(email)) {
        Swal.fire('Validation Error', 'Please enter a valid email address', 'error');
        return false;
    }

    return true; // Allow form submission if all fields are valid

    

}

</script>
  </body>
</html>